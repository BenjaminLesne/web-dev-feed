...
const ALLOWED_REGEX = /\b(javascript|js|react|node\.?js|typescript|ts|html5?|css3?|web\s*dev(elopment)?|frontend|front-end|backend|back-end|npm|yarn|webpack|vite|babel|eslint|docker|kubernetes|angular|vue\.?js|svelte|next\.?js|nuxt\.?js|express\.?js|deno|graphql|rest\s*api|typescript|tailwind\.?css|redux|mobx|storybook|jest|cypress|playwright|webpack|parcel|gulp|electron|pwa|progressive\s*web\s*app|websockets?|ssr|server\s*side\s*rendering|spa|single\s*page\s*application|es6|es2015|es2016|es2017|es2018|es2019|es2020|es2021|es2022|es2023|async|await|promise|closure|prototype|functional\s*programming|reactjs|reactnative|typescript|ts|angular\.?js|vuejs|emberjs|backbone\.?js|3\.?js|d3\.?js|rxjs|cli|html\s*css|web\s*standards|browser\s*api|web\s*components|alpine\.?js|solid\.?js)\b/i;
